-- Constants
local PLAYER_SIZE = 50
local PLAYER_SPEED = 200
local OBJECT_SIZE = 30
local OBJECT_SPEED_MIN = 100
local OBJECT_SPEED_MAX = 300
local OBJECT_SPAWN_RATE = 0.5
local BULLET_SIZE = 10
local BULLET_SPEED = 500
local BULLET_RELOAD_TIME = 1

-- Game state variables
local player = { x = 0, y = 0 }
local objects = {}
local bullets = {}
local score = 0
local lives = 3
local gameStarted = false
local gameOver = false
local lastBulletTime = 0

-- Load assets
local shootingSound
local bulletHitSound

function love.load()
    shootingSound = love.audio.newSource("srola.wav", "static")
    bulletHitSound = love.audio.newSource("gartyma.wav", "static")

    love.window.setTitle("Object Shooter")
    love.window.setMode(1360, 720, { resizable = true })
    love.graphics.setBackgroundColor(0.2, 0.2, 0.2)
    love.keyboard.setKeyRepeat(true)

    startGame()
end

-- Start the game
function startGame()
    player.x = love.graphics.getWidth() / 2 - PLAYER_SIZE / 2
    player.y = love.graphics.getHeight() - PLAYER_SIZE - 10

    objects = {}
    bullets = {}
    score = 0
    lives = 3
    gameStarted = false
    gameOver = false
    lastBulletTime = 0
end

-- Update the game state
function love.update(dt)
    if not gameStarted then
        return
    end

    -- Player movement
    if love.keyboard.isDown("w") and player.y > 0 then
        player.y = player.y - PLAYER_SPEED * dt
    elseif love.keyboard.isDown("s") and player.y < love.graphics.getHeight() - PLAYER_SIZE then
        player.y = player.y + PLAYER_SPEED * dt
    end

    if love.keyboard.isDown("a") and player.x > 0 then
        player.x = player.x - PLAYER_SPEED * dt
    elseif love.keyboard.isDown("d") and player.x < love.graphics.getWidth() - PLAYER_SIZE then
        player.x = player.x + PLAYER_SPEED * dt
    end

    -- Object spawning
    if not gameOver and math.random() < OBJECT_SPAWN_RATE * dt then
        local object = {}
        object.x = math.random(love.graphics.getWidth() - OBJECT_SIZE)
        object.y = -OBJECT_SIZE
        object.speed = math.random(OBJECT_SPEED_MIN, OBJECT_SPEED_MAX)
        table.insert(objects, object)
    end

    -- Object movement and collision
    for i = #objects, 1, -1 do
        local object = objects[i]
        if object then
            object.y = object.y + object.speed * dt

            -- Check collision with player
            if checkCollision(player, object) then
                lives = lives - 1
                if lives <= 0 then
                    gameOver = true
                end
                table.remove(objects, i)
            end

            -- Check collision with bullets
            for j = #bullets, 1, -1 do
                local bullet = bullets[j]
                if bullet and checkCollision(bullet, object) then
                    table.remove(objects, i)
                    table.remove(bullets, j)
                    score = score + 100
                    bulletHitSound:play()
                    break
                end
            end
        end
    end

    -- Bullet movement
    for i = #bullets, 1, -1 do
        local bullet = bullets[i]
        bullet.y = bullet.y - BULLET_SPEED * dt

        -- Remove bullets that go off-screen
        if bullet.y < 0 then
            table.remove(bullets, i)
        end
    end

    -- Bullet reload time
    lastBulletTime = lastBulletTime + dt
end

-- Draw the game objects
function love.draw()
    love.graphics.setColor(1, 1, 1)

    -- Draw player
    love.graphics.rectangle("fill", player.x, player.y, PLAYER_SIZE, PLAYER_SIZE)

    -- Draw objects
    for _, object in ipairs(objects) do
        love.graphics.rectangle("fill", object.x, object.y, OBJECT_SIZE, OBJECT_SIZE)
    end

    -- Draw bullets
    for _, bullet in ipairs(bullets) do
        love.graphics.rectangle("fill", bullet.x, bullet.y, BULLET_SIZE, BULLET_SIZE)
    end

    -- Draw score and lives
    love.graphics.setColor(1, 1, 1)
    love.graphics.print("Score: " .. score, 10, 10)
    love.graphics.print("Lives: " .. lives, love.graphics.getWidth() - 70, 10)

    -- Draw start screen
    if not gameStarted then
        love.graphics.printf("Press Enter to Play", 0, love.graphics.getHeight() / 2, love.graphics.getWidth(), "center")
    end

    -- Draw game over screen
    if gameOver then
        love.graphics.printf("Game Over\n\nYour Score: " .. score .. "\n\nPress Enter to Play Again", 0, love.graphics.getHeight() / 2, love.graphics.getWidth(), "center")
    end
end

-- Start the game when Enter key is pressed
function love.keypressed(key)
    if key == "return" and not gameStarted then
        gameStarted = true
    elseif key == "return" and gameOver then
        startGame()
    elseif key == "space" and gameStarted and not gameOver and lastBulletTime >= BULLET_RELOAD_TIME then
        local bullet = {}
        bullet.x = player.x + PLAYER_SIZE / 2 - BULLET_SIZE / 2
        bullet.y = player.y
        table.insert(bullets, bullet)
        shootingSound:play()
        lastBulletTime = 0
    end
end

-- Check collision between two rectangles
function checkCollision(rect1, rect2)
    return rect1.x < rect2.x + OBJECT_SIZE and
           rect2.x < rect1.x + PLAYER_SIZE and
           rect1.y < rect2.y + OBJECT_SIZE and
           rect2.y < rect1.y + PLAYER_SIZE
end
